...
<template>
  <div class="bg-primary w-full xl:hidden">
    <button class="bg-primary-light p-2 px-6 pl-5 text-white rounded-br-2xl" @click="handleSidebarOpen">
  ...
  <BaseToast :show="showToast" variant="success">
    <template #default>{{ toastTitle }}</template>
    <template #description>{{ toastMessage }}</template>
  </BaseToast>
</template>
